...
async function testModel(model) {
  return new Promise((resolve) => {
    const data = JSON.stringify({
      model: model,
      messages: [{ role: 'user', content: '回复"你好"两个字即可' }],
      max_tokens: 20
    });
    const req = http.request({
      ...
      res.on('end', () => {
        try {
          ...
          else if (j.choices) console.log(`  OK: ${j.choices[0]?.message?.content?.substring(0, 100)}`);
          else console.log(`  UNEXPECTED: ${JSON.stringify(j).substring(0, 200)}`);
...
(async () => {
  ...
  await testModel('zhipu:glm-4.6v-flash');
